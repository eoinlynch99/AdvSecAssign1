...
public class AESKeyExp
{
	// Adds the rcon values to an array
	protected static final int RconVal[] = 
		{
			0x01000000,
	...
	private static int toWord(byte by1, byte by2, byte by3, byte by4) 
	{
        int wrd1 = 0;
        
        
        wrd1 ^= ((int) by1) << 24;
        wrd1 ^= (((int) by2) & 0x000000ff) << 16;
        wrd1 ^= (((int) by3) & 0x000000ff) << 8;
        wrd1 ^= (((int) by4) & 0x000000ff);
	...
	private static int undWord(int word) 
	{
        int wrd2 = 0;
        
        
        wrd2 ^= (int) STransform((byte) (word >>> 24)) & 0x000000ff;
        wrd2 <<= 8;
        wrd2 ^= (int) STransform((byte) ((word & 0xff0000) >>> 16)) &
                0x000000ff;
        wrd2 <<= 8;
        wrd2 ^= (int) STransform((byte) ((word & 0xff00) >>> 8)) &
                0x000000ff;
        wrd2 <<= 8;
        wrd2 ^= (int) STransform((byte) (word & 0xff)) & 0x000000ff;
	...
	private static int rtWord(int wrd1) 
	{
		
        return (wrd1 << 8) ^ ((wrd1 >> 24) & 0x000000ff);
    }
	
	
	
	private static byte STransform(byte val1) 
	{
        byte great = 0, small = 0;
        
        great = (byte) ((byte) (val1 >> 4) & 0x0f);
        small = (byte) (val1 & 0x0f);
        
        return sarray[great][small];
    }
	
	
	
	public void keyExpansion(byte AESkey[], int wrd1[]) 
	{
        ...
        for(int i = 0; i < 4; i++)
        {
        	wrd1[i] = toWord( AESkey[4 * i], AESkey[4 * i + 1], AESkey[4 * i + 2], AESkey[4 * i + 3]);
        }
        ...
        for(int i =4;i <44;i++) 
        {
        	...
        	if(i % 4 == 0)
        	{
        		temp = undWord(rtWord (temp)) ^ RconVal[i / 4];
        	}
        	
        	wrd1[i] = wrd1[i - 4] ^ temp;
        }
    }

	
	
	protected static final byte[][] sarray = 
		{ 
			{
				(byte) 0x63, (byte) 0x7c,
